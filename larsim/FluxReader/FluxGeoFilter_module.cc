...
//JAZ geo implementation
#include "larsim/GeoKT/GeoVector.h"
#include "larsim/GeoKT/GeoAABox.h"
#include "larsim/GeoKT/GeoHalfLine.h"
#include "larsim/GeoKT/GeoAlgo.h"
  ...
  */
  ...
  for(unsigned int inu = 0; inu < mclist.size(); inu++){
    ...
    geoalgo::HalfLine ray(mclist[inu].GetNeutrino().Nu().Vx(),
			  ...
			  mclist[inu].GetNeutrino().Nu().Vz(),
			  mclist[inu].GetNeutrino().Nu().Px(),
			  mclist[inu].GetNeutrino().Nu().Py(),
			  mclist[inu].GetNeutrino().Nu().Pz());
