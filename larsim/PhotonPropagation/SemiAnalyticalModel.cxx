...
SemiAnalyticalModel::SemiAnalyticalModel(fhicl::ParameterSet VUVHits,
    ...
    : fVUVHitsParams(VUVHits),
      fVISHitsParams(VISHits), fISTPC{*(lar::providerFrom<geo::Geometry>())},
      fGeom{*(lar::providerFrom<geo::Geometry>())}, fNTPC(fGeom.NTPC()),
      fActiveVolumes(fISTPC.extractActiveLArVolume(fGeom)),
      fcathode_centre{fGeom.TPC(0, 0).GetCathodeCenter().X(),
                      fActiveVolumes[0].CenterY(), fActiveVolumes[0].CenterZ()},
      fanode_centre{fGeom.TPC(0, 0).FirstPlane().GetCenter().X(),
                    fActiveVolumes[0].CenterY(), fActiveVolumes[0].CenterZ()},
      nOpDets(fGeom.NOpDets()), fvuv_absorption_length(VUVAbsorptionLength()),
...
void SemiAnalyticalModel::Initialization() {
  ...
  fApplyFieldCageTransparency =
      fVUVHitsParams.get<bool>("ApplyFieldCageTransparency", false);
  fFieldCageTransparencyLateral =
      fVUVHitsParams.get<double>("FieldCageTransparencyLateral", 1.0);
  fFieldCageTransparencyCathode =
      fVUVHitsParams.get<double>("FieldCageTransparencyCathode", 1.0);
  ...
  if (fIsFlatPDCorr) {
    fGHvuvpars_flat =
        fVUVHitsParams.get<std::vector<std::vector<double>>>("GH_PARS_flat");
    fborder_corr_angulo_flat =
        fVUVHitsParams.get<std::vector<double>>("GH_border_angulo_flat");
    fborder_corr_flat =
        fVUVHitsParams.get<std::vector<std::vector<double>>>("GH_border_flat");
  }
  if (fIsFlatPDCorrLat) {
    fGHvuvpars_flat_lateral =
        fVUVHitsParams.get<std::vector<std::vector<double>>>(
            "GH_PARS_flat_lateral");
    fborder_corr_angulo_flat_lateral = fVUVHitsParams.get<std::vector<double>>(
  ...
  if (fIsDomePDCorr) {
    fGHvuvpars_dome =
        fVUVHitsParams.get<std::vector<std::vector<double>>>("GH_PARS_dome");
    fborder_corr_angulo_dome =
        fVUVHitsParams.get<std::vector<double>>("GH_border_angulo_dome");
    fborder_corr_dome =
        fVUVHitsParams.get<std::vector<std::vector<double>>>("GH_border_dome");
  }
  ...
  if (fDoReflectedLight) {
    ...
    if (fIsFlatPDCorr) {
      fvis_distances_x_flat =
          fVISHitsParams.get<std::vector<double>>("VIS_distances_x_flat");
      fvis_distances_r_flat =
          fVISHitsParams.get<std::vector<double>>("VIS_distances_r_flat");
      fvispars_flat =
          fVISHitsParams.get<std::vector<std::vector<std::vector<double>>>>(
              "VIS_correction_flat");
    }
    if (fIsDomePDCorr) {
      fvis_distances_x_dome =
          fVISHitsParams.get<std::vector<double>>("VIS_distances_x_dome");
      fvis_distances_r_dome =
          fVISHitsParams.get<std::vector<double>>("VIS_distances_r_dome");
      fvispars_dome =
          fVISHitsParams.get<std::vector<std::vector<std::vector<double>>>>(
              "VIS_correction_dome");
    }
  ...
  if (fIncludeAnodeReflections) {
    ...
    if (fIsFlatPDCorrLat) {
      fvis_distances_x_flat_lateral = fVISHitsParams.get<std::vector<double>>(
          "VIS_distances_x_flat_lateral");
      fvis_distances_r_flat_lateral = fVISHitsParams.get<std::vector<double>>(
...
int SemiAnalyticalModel::VUVAbsorptionLength() const {
  ...
  int vuv_absorption_length = std::round(interpolate(
      x_v, y_v, 9.7, false)); // 9.7 eV: peak of VUV emission spectrum   // TO
                              // DO UNHARDCODE FOR XENON
...
void SemiAnalyticalModel::detectedDirectVisibilities(
    std::vector<double> &DetectedVisibilities,
  ...
  for (size_t const OpDet : util::counter(nOpDets)) {
    ...
    const SemiAnalyticalModel::OpticalDetector op{
        fOpDetHeight[OpDet], fOpDetLength[OpDet], fOpDetCenter[OpDet],
        fOpDetType[OpDet], fOpDetOrientation[OpDet]};
...
double SemiAnalyticalModel::VUVVisibility(geo::Point_t const &ScintPoint,
  ...
  // PMTs (disk)
    ...
    const double zy_offset =
        std::sqrt(relative.Y() * relative.Y() + relative.Z() * relative.Z());
  ...
  double visibility_geo = std::exp(-1. * distance / fvuv_absorption_length) *
                          (solid_angle / (4 * CLHEP::pi));
  ...
  double r = std::hypot(ScintPoint.Y() - fcathode_centre[1],
                        ScintPoint.Z() - fcathode_centre[2]);
  ...
  double s3 = 0;
  // flat PDs
  if ((opDet.type == 0 || opDet.type == 2) &&
      (fIsFlatPDCorr || fIsFlatPDCorrLat)) {
    if (opDet.orientation == 1 && fIsFlatPDCorrLat) { // laterals
      ...
      s1 = interpolate(fborder_corr_angulo_flat, fborder_corr_flat[0], theta,
                       true);
      s2 = interpolate(fborder_corr_angulo_flat, fborder_corr_flat[1], theta,
                       true);
      s3 = interpolate(fborder_corr_angulo_flat, fborder_corr_flat[2], theta,
                       true);
    } else {
      throw cet::exception("SemiAnalyticalModel")
  ...
  // dome PDs
    ...
    s1 = interpolate(fborder_corr_angulo_dome, fborder_corr_dome[0], theta,
                     true);
    s2 = interpolate(fborder_corr_angulo_dome, fborder_corr_dome[1], theta,
                     true);
    s3 = interpolate(fborder_corr_angulo_dome, fborder_corr_dome[2], theta,
                     true);
  } else {
    throw cet::exception("SemiAnalyticalModel")
...
void SemiAnalyticalModel::detectedReflectedVisibilities(
    ...
    geo::Point_t const &ScintPoint, bool AnodeMode) const {
  ...
  if (AnodeMode) {
    ...
    ScintPoint_relative.SetCoordinates(
        std::abs(ScintPoint.X() - fanode_plane_depth),
  ...
  double solid_angle_cathode =
      Rectangle_SolidAngle(plane_dimensions, ScintPoint_relative, 0);
  ...
  double cathode_visibility_geo =
      std::exp(-1. * distance_cathode / fvuv_absorption_length) *
  ...
  for (size_t const OpDet : util::counter(nOpDets)) {
    ...
    // set detector struct for solid angle function
    const OpticalDetector op{fOpDetHeight[OpDet], fOpDetLength[OpDet],
                             fOpDetCenter[OpDet], fOpDetType[OpDet],
                             fOpDetOrientation[OpDet]};

    ReflDetectedVisibilities[OpDet] = VISVisibility(
        ScintPoint, op, cathode_visibility_rec, hotspot, AnodeMode);
  }
}
...
double SemiAnalyticalModel::VISVisibility(geo::Point_t const &ScintPoint,
  ...
  if (opDet.type == 0) {
    ...
    solid_angle_detector = Rectangle_SolidAngle(
        Dims{opDet.h, opDet.w}, abs_emission_relative, opDet.orientation);
  }
  // PMTS (dome)
  else if (opDet.type == 1) {
  ...
  // PMTs (disk)
    ...
    const double zy_offset =
        std::sqrt(emission_relative.Y() * emission_relative.Y() +
  ...
  if ((opDet.type == 0 || opDet.type == 2) &&
    ...
    if (opDet.orientation == 0 && fIsFlatPDCorr) {
      border_correction =
          interpolate2(fvis_distances_x_flat, fvis_distances_r_flat,
  ...
  // dome PDs
    ...
    border_correction = interpolate2(
        fvis_distances_x_dome, fvis_distances_r_dome, fvispars_dome, d_c, r, k);
  } else {
    throw cet::exception("SemiAnalyticalModel")
        << "Error: Invalid optical detector shape requested or corrections are "
           "missing - configuration error in semi-analytical model."
  ...
  if (AnodeMode)
    border_correction *= fAnodeReflectivity;
...
double SemiAnalyticalModel::Gaisser_Hillas(const double x,
...
double SemiAnalyticalModel::Disk_SolidAngle(const double d, const double h,
...
double SemiAnalyticalModel::Rectangle_SolidAngle(const double a, const double b,
...
double
SemiAnalyticalModel::Rectangle_SolidAngle(Dims const &o, geo::Vector_t const &v,
  ...
  if (isDefinitelyGreaterThan(d1, o.h * .5) &&
    ...
    double to_return =
        (Rectangle_SolidAngle(2. * (A + o.h), 2. * (B + o.w), d2) -
  ...
  if ((d1 <= o.h * .5) && (std::abs(v.Z()) <= o.w * .5)) {
    ...
    double B = -std::abs(v.Z()) + o.w * .5;
    double to_return =
        (Rectangle_SolidAngle(2. * (o.h - A), 2. * (o.w - B), d2) +
  ...
  if (isDefinitelyGreaterThan(d1, o.h * .5) && (std::abs(v.Z()) <= o.w * .5)) {
    ...
    double B = -std::abs(v.Z()) + o.w * .5;
    double to_return =
        (Rectangle_SolidAngle(2. * (A + o.h), 2. * (o.w - B), d2) -
         Rectangle_SolidAngle(2. * A, 2. * (o.w - B), d2) +
         Rectangle_SolidAngle(2. * (A + o.h), 2. * B, d2) -
         Rectangle_SolidAngle(2. * A, 2. * B, d2)) *
        .25;
  ...
  if ((d1 <= o.h * .5) && isDefinitelyGreaterThan(std::abs(v.Z()), o.w * .5)) {
    ...
    double B = std::abs(v.Z()) - o.w * .5;
    double to_return =
        (Rectangle_SolidAngle(2. * (o.h - A), 2. * (B + o.w), d2) -
         Rectangle_SolidAngle(2. * (o.h - A), 2. * B, d2) +
         Rectangle_SolidAngle(2. * A, 2. * (B + o.w), d2) -
         Rectangle_SolidAngle(2. * A, 2. * B, d2)) *
        .25;
...
double SemiAnalyticalModel::Omega_Dome_Model(const double distance,
  ...
  double par1[9] = {0,        0,       0.19569, 0.300449, 0.555598,
                    0.854939, 1.39166, 2.19141, 2.57732};
  ...
  if (distance >= d_break) {
    ...
    double ratio_square =
        (R_apparent_far * R_apparent_far) / (distance * distance);
    return (2 * CLHEP::pi * (1 - std::sqrt(1 - ratio_square)));
  } else {
    double R_apparent_close = b - par0[j];
    double ratio_square =
        (R_apparent_close * R_apparent_close) / (distance * distance);
    return (2 * CLHEP::pi * (1 - std::sqrt(1 - ratio_square)));
  }
}
...
bool SemiAnalyticalModel::isOpDetInSameTPC(
